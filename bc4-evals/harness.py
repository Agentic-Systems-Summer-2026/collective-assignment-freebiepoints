...
CAPSTONE_DIR = pathlib.Path(__file__).resolve().parents[1] / "capstone"
USER_ARTIFACT_PATH = CAPSTONE_DIR / "user_changelog.md"
TECH_ARTIFACT_PATH = CAPSTONE_DIR / "technical_patch_notes.md"


def _cleanup_doc_artifacts() -> None:
    """Best-effort cleanup of docs generated by target() for eval runs."""
    for path in (USER_ARTIFACT_PATH, TECH_ARTIFACT_PATH):
        try:
            path.unlink()
...
def _read_doc_artifact(path: pathlib.Path) -> str:
    """Read artifact content if present; return empty string otherwise."""
    try:
...
def target(prompt: str) -> str:
    ...
    run_agent_slice(commit_hash=commit_hash, commit_message=commit_message)

    user_docs = _read_doc_artifact(USER_ARTIFACT_PATH)
    technical_docs = _read_doc_artifact(TECH_ARTIFACT_PATH)
    if not user_docs and not technical_docs:
        return "ERROR: no documentation artifacts were produced by run_agent_slice"
